...
function BotaoGoogle() {
  ...
  return (
    <>
      <button onClick={handleGoogle} disabled={carregando}
        style={{ width: '100%', padding: '0.75rem', backgroundColor: 'white', color: '#1a1a1a', border: '1px solid #d4ddc8', borderRadius: 6, fontWeight: 600, fontSize: '0.9rem', cursor: carregando ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: '1.25rem', opacity: carregando ? 0.7 : 1 }}>
        <IconGoogle />
        {carregando ? 'Redirecionando...' : 'Cadastrar com Google'}
      </button>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
        <div style={{ flex: 1, height: 1, backgroundColor: '#d4ddc8' }} />
        <span style={{ fontSize: '0.78rem', color: '#6a7a6a' }}>ou preencha os dados</span>
        <div style={{ flex: 1, height: 1, backgroundColor: '#d4ddc8' }} />
      </div>
    </>
  )
